...
jobject makeResult(JNIEnv* env, mln_plugin_status status, const char* message) {
    jclass type = env->FindClass("org/maplibre/plugins/heatmap/HeatmapLayerPlugin$NativeResult");
...
Java_org_maplibre_plugins_heatmap_HeatmapLayerPlugin_nativeRegister(JNIEnv* env, jclass, jlong functionAddress) {
    char error[512]{};
    if (!functionAddress) {
    ...
    const auto status = mln_heatmap_layer_register(registerPlugin, error, sizeof(error));
