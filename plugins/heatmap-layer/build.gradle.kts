...
android {
    namespace = "org.maplibre.plugins.heatmap"
    compileSdk = 34
        ...
        externalNativeBuild {
            cmake {
                ...
                arguments += listOf(
                    ...
                    "-DMLN_HEATMAP_PLUGIN_VERSION=${pluginVersion.get()}"
                )
            }
        }
    }
...
publishing {
    publications {
        register<MavenPublication>("release") {
            ...
            artifactId = "heatmap-layer"
            version = project.version.toString()
            afterEvaluate { from(components["release"]) }
            pom {
                name.set("MapLibre Heatmap Layer")
                description.set("Source-bound heatmap style layer implemented through the MapLibre Native plugin API")
