        ...
        externalNativeBuild {
            cmake {
                ...
                arguments += listOf(
                    ...
                    "-DMLN_NGON_PLUGIN_VERSION=${pluginVersion.get()}"
                )
            }
        }
    }
...
publishing {
    publications {
        register<MavenPublication>("release") {
            ...
            artifactId = "ngon-layer"
            version = project.version.toString()
            afterEvaluate { from(components["release"]) }
            pom {
                name.set("MapLibre Ngon Layer")
                description.set("Source-bound point ngon style layer for MapLibre Native")
