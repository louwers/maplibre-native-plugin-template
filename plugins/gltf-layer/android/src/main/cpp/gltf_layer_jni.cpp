...
jobject makeResult(JNIEnv* env, mln_plugin_status status, const char* message) {
    jclass type = env->FindClass("org/maplibre/plugins/gltf/GltfLayerPlugin$NativeResult");
...
Java_org_maplibre_plugins_gltf_GltfLayerPlugin_nativeRegister(JNIEnv* env, jclass, jlong functionAddress) {
    char error[512]{};
    if (!functionAddress) {
    ...
    const auto status = mln_gltf_layer_register(registerPlugin, error, sizeof(error));
